...
extension TerminalController: ControlNotificationContext {
    func controlNotificationCreate(
        ...
        deliverNotificationSynchronously(
            ...
            subtitle: subtitle,
            body: body,
            source: Self.controlNotificationSource(source)
        )
    ...
    /// Maps the wire `source`/`notification_source` label onto the app's
    /// notification-source enum (the legacy `v2NotificationSource` mapping).
    static func controlNotificationSource(_ raw: String?) -> TerminalNotificationSource {
        switch raw?.replacingOccurrences(of: "-", with: "_").lowercased() {
