...
| 机器重启 / liveshd 被 kill -9 | bridge 收到 EOF，exit 69 | 全部丢失 | 启动时跑 `livesh --open` 会拿到 exit 66，**显式提示用户** |
...
**Note on architecture deviation from §5.1**: Ghostty owns the pane fork+exec via
`surfaceConfig.command` (C string). cmux cannot inject an inheritable fd 3 to the spawned
child without invasive Ghostty API changes. So the cmux-side implementation here uses
existing vault.agents argv-scan detection (Path B's mechanism, already wired) to capture the
sh_id rather than the fd-3 wire contract. The pane schema does not need a new `liveShellId`
field — `SurfaceResumeBindingSnapshot.checkpointId` + `kind="livesh"` already does the job.

- [ ] 给 manaflow-ai/cmux 提 issue：propose live-shell backend。issue 草稿见 §10。
- [x] Schema 扩展：`terminal.shellBackend` (新 setting, default `direct`)。
      pane layout 复用 `SurfaceResumeBindingSnapshot.checkpointId` (无需新增字段)。
      见 `Sources/LiveShellSettings.swift`, `Sources/KeyboardShortcutSettingsFileStore.swift:493+`,
      `Sources/CmuxSettingsJSONPathSupport.swift:70-74`。
- [x] Pane spawn fallback：当 `terminal.shellBackend = "livesh"` 且未配置 Ghostty `command =` 时，
      `GhosttyTerminalView` 自动用 livesh 作 pane 命令。见 `Sources/GhosttyTerminalView.swift:5836+`。
- [x] `liveshctl` async wrapper：`Sources/LiveShellControl.swift` 提供 `list()` / `kill(sessionId:)`
      / `gc()` / exit code 常量，作为 UI 层的基础。
- [x] Command palette 入口：`palette.listLiveShells`（NSAlert 列出所有 sessions）、
      `palette.gcLiveShells`（liveshctl gc）、`palette.killFocusedLiveShell`（读 focused 面板
      `resumeBinding.checkpointId` 调 liveshctl kill）。见
      `Sources/LiveShellCommandPaletteActions.swift`, `Sources/ContentView.swift:7217+`。
- [x] Close vs Kill 区分：cmux 现有 "Close pane" 已经只 SIGHUP livesh bridge —— livesh daemon 里的
      shell 仍存活。新增 "Kill Live Shell of Focused Pane" 调 `liveshctl kill <sh_id>` 显式销毁。
      menu / 快捷键 surface 延后（palette 作为 canonical 入口，遵循 CLAUDE.md shared-behavior policy）。
- [x] Exit code 66 (shell lost) / 69 (daemon unavailable) 提示：钩进 Ghostty
      `GHOSTTY_ACTION_SHOW_CHILD_EXITED` 回调，先读 `action.action.child_exited.exit_code`，
      若 panel 的 resumeBinding.kind == "livesh" 且 code 匹配 66/69，弹 NSAlert 给原因；
      66 时同时清掉 stale resumeBinding，防止 cmux 重启时尝试恢复已死的 session。其他 binary
      若也用 sysexits.h 66/69，走默认 close 路径，不弹 livesh 文案。见
      `Sources/LiveShellChildExitHandler.swift`, `Sources/GhosttyTerminalView.swift:4061+`。
- [ ] Settings UI panel：在 Settings 加 shellBackend / liveshExecutablePath 选项。
- [ ] Pane chrome：sh_xxxx + attached/detached 指示器。
- [ ] 关键设计点：与 vault.agents 模型的关系（livesh 应不应该也走 vault，还是独立 backend？倾向独立 backend，因为 livesh 不是 agent）。
      当前实现是混合：detection 走 vault.agents (复用现有 argv 扫描)，但 UX 入口 (kill / attach / pane chrome) 走独立的 `LiveShellControl`。
