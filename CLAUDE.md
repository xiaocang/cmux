...
- **Semantic routing policy:** For assistant/sprite/sort routing decisions, prefer the semantic router as the source of truth for intent and route selection. Keyword checks are acceptable only for exact commands, very early obvious short-circuits, or conservative fallback behavior; do not keep expanding keyword lists as the primary way to decide sorting paths.
...
- **Sprite assistant floating position:** user-dragged sprite assistant panels must be allowed to live outside the parent window bounds. Clamp only initial or automatic layout positions when the user has not manually placed the sprite; do not reuse autocomplete/popover visibility fixes to constrain sprite dragging. Switching the sprite to mini/edge-recovery mode must be based only on the physical display visible frames (`NSScreen.visibleFrame` across screens), never on the cmux parent window/content/frame; parent overflow may be debug-logged but must not drive mini mode or clamping.
