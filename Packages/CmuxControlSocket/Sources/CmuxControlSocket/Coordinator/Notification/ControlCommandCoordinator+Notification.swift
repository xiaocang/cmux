...
extension ControlCommandCoordinator {
    ...
    func notificationCreate(_ params: [String: JSONValue]) -> ControlCallResult {
        ...
        let resolution = context?.controlNotificationCreate(
            ...
            subtitle: subtitle,
            body: body,
            source: notificationSource(params)
    ...
    /// The raw `source`/`notification_source` label carried by a create
    /// request, if any. The app maps it onto its notification-source enum.
    private func notificationSource(_ params: [String: JSONValue]) -> String? {
        rawString(params, "source") ?? rawString(params, "notification_source")
    }
