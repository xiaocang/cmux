...
public struct ControlNotificationSnapshot: Sendable, Equatable {
    ...
    /// The notification's source label (the app's notification-source enum
    /// raw value, e.g. `agent` / `monitor`), written as `source`.
    ...
    ///   - source: The notification-source raw value (`agent` when absent).
